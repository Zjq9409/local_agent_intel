...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            ...
            {
                name: "read_text_file",
                description: "Read the complete contents of a plain text file from the file system. " +
                            "This tool handles various text encodings and provides detailed error messages " +
                            "if the file cannot be read. **Only use for plain text files (e.g., .txt, .log); " +
                            "do NOT use for Word (.docx), Excel (.xlsx), PowerPoint (.pptx) or other binary documents.** " +
                            "Use the 'head' parameter to read only the first N lines of a file, or the 'tail' parameter " +
                            "to read only the last N lines of a file. Operates strictly on text files within allowed directories.",
                inputSchema: zodToJsonSchema(ReadTextFileArgsSchema),
            },
            // {
